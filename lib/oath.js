void function () {
   ...
   */
  /*Error for undefined or required variables
   * @class
   * */
  var RefError = function (m){
    var err = new ReferenceError(m);
  ...
  function Reconciler (promises,listener) {
    promises.length < 2 && new RefError('Expected at least two Promises are required!: new Reconciler([promise x 2])').throw();
    ...
    this.results      = function () {
      return this.promises.reduce(function (results,promise) {
        // debug('promise.value',promise.value,promise.rejected ? ' rejected': ' resolved')
    ...
    this.promises.forEach(function(promise){
      promise
        .then (this.listener)
        .catch(this.listener);
      // debug('promise._okay.handlers',promise.target.obj,typeof promise._okay.handlers[0])
      // debug('promise._fail.handler ',promise.target.obj,typeof promise._fail.handler)
      promise.target.delay  = ~~((++this.coefficient/Math.E)*10);
      typeof promise.target.obj === 'undefined' && new RefError('Each promise has to have a target object!: promise.target.obj').throw();
    },this);

    /*Initialize each promise in pool
     * @method reconciler.reconcile*/
    this.reconcile    = function () {
      this.listener || new RefError('Expected a promises listener to be defined!: reconciler.listener').throw();
      this.promises.forEach(function (promise) {
